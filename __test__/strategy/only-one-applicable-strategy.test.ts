...
describe('OnlyOneApplicableStrategy: permission.client.delete', () => {
  const policies = new AbilityDSLParser(dsl).parse();
  const resolver = new AbilityResolver(policies, OnlyOneApplicableStrategy);
  ...
  // 1. Admin
  //
  test('admin can delete any client (only one applicable)', () => {
  ...
  // 2. Manager
  //
  test('manager cannot delete ООО/ПАО/ОАО (two applicable → deny)', () => {
  ...
  test('manager can delete other clients (only one applicable)', () => {
    const user = makeUser('manager');
  ...
  // 3. Dispatcher
  //
  test('dispatcher cannot delete old IP (two applicable → deny)', () => {
  ...
  test('dispatcher can delete fresh IP (only one applicable)', () => {
    const user = makeUser('dispatcher');
  ...
  // 4. Operator
  //
  test('operator cannot delete blocked clients (two applicable → deny)', () => {
  ...
  test('operator can delete unblocked clients (only one applicable)', () => {
    const user = makeUser('operator');
