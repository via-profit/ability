...
import AnyPermitStrategy from '../../src/strategy/AnyPermitStrategy';
...
describe('AnyPermitStrategy: permission.client.delete', () => {
  const policies = new AbilityDSLParser(dsl).parse();
  const resolver = new AbilityResolver(policies, AnyPermitStrategy);
  ...
  // 2. Manager
  //
  test('manager CAN delete ООО/ПАО/ОАО (permit wins over deny)', () => {
  ...
  // 3. Dispatcher
  //
  test('dispatcher CAN delete old IP (permit wins over deny)', () => {
  ...
  // 4. Operator
  //
  test('operator CAN delete blocked clients (permit wins over deny)', () => {
