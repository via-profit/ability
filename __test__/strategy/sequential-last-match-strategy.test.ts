...
import SequentialLastMatchStrategy from '../../src/strategy/SequentialLastMatchStrategy';
...
describe('SequentialLastMatchStrategy: permission.client.delete', () => {
  const policies = new AbilityDSLParser(dsl).parse();
  const resolver = new AbilityResolver(policies, SequentialLastMatchStrategy);
  ...
  // 1. Admin
  //
  test('admin can delete any client (last match is permit)', () => {
  ...
  // 2. Manager
  //
  test('manager CAN delete ООО/ПАО/ОАО (last match is permit)', () => {
  ...
  test('manager can delete other clients (last match is permit)', () => {
    const user = makeUser('manager');
  ...
  // 3. Dispatcher
  //
  test('dispatcher CAN delete old IP (last match is permit)', () => {
  ...
  test('dispatcher can delete fresh IP (last match is permit)', () => {
    const user = makeUser('dispatcher');
  ...
  // 4. Operator
  //
  test('operator CAN delete blocked clients (last match is permit)', () => {
  ...
  test('operator can delete unblocked clients (last match is permit)', () => {
    const user = makeUser('operator');
