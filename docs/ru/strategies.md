...
|-----------------------------------|------------------------------------------------------------|----------------------------------------------------------------------|
| **`DenyOverridesStrategy`**       | Если есть хотя бы одна `deny` → `deny`, иначе `permit`     | **Рекомендуемая**. Безопасный подход — запрет перевешивает разрешение |
...
> [!NOTE]
> Во всех встроенных стратегиях действует общее правило: если ни одна политика не совпала, итог — `deny`.
> У `PriorityStrategy` при равных приоритетах побеждает политика, объявленная раньше.
...
Стратегии по умолчанию нет — её всегда нужно передать в конструктор резолвера явно. Рекомендуется
`DenyOverridesStrategy` — она наиболее безопасна.
