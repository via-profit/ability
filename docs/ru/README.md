...
В отличие от классических ABAC‑моделей, где политики работают по принципу *«совпала → применилась, не совпала →
игнорируется»*, в Ability используется **упрощённая и более предсказуемая state‑machine модель**:

- выполняются **все** подходящие политики в порядке их объявления,
- каждая политика может **установить** состояние (`permit` или `deny`),
- каждая политика может **сбросить** состояние, если её условия не выполнены,
- итог определяется **последней обработанной политикой**, а не только совпавшей.
...
Вот аккуратно обновлённая версия раздела **«Основные положения»**, в которой отражена новая **state‑machine модель**, но
структура и стиль твоего текста полностью сохранены.  
Я добавил только то, что действительно важно знать пользователю, чтобы он не ожидал классического ABAC‑поведения.
...
1. **Резолвер (`AbilityResolver`) работает по принципу `Default Deny`.**  
   Если ни одна политика не установила итоговое состояние, результат будет `deny`  
   ([подробнее здесь](#решение-проблем)).  
   Чтобы избежать неожиданного `deny`, убедитесь, что существует хотя бы одна `permit`‑политика, которая может совпасть.
   Только после этого добавляйте `deny`‑политики.

2. **Политики обрабатываются последовательно, сверху вниз.**  
   В отличие от классических ABAC‑моделей, где `mismatch` просто игнорируется, в Ability используется **state‑machine
   модель**:

- `match` устанавливает состояние (`permit` → allow, `deny` → deny),
- `mismatch` **сбрасывает состояние в neutral**,
- итог определяется **последней обработанной политикой**, а не только совпавшей.

3. **Правила внутри политики также выполняются последовательно.**

4. **В группе правил (`RuleSet`) с оператором `all` выполнение прекращается при первом `mismatch`.**  
   В `any` — при первом `match`.

5. **Для составления политик используйте [DSL](#dsl)** — это проще и удобнее.

6. **Для хранения политик на сервере используйте JSON.**  
   Политики можно экспортировать в JSON и импортировать обратно.

7. **Ориентируйтесь на принцип: если разрешение не выдано явно → доступ запрещён.**  
   Это естественное следствие модели `Default Deny` и state‑machine поведения.
...
Вот обновлённый раздел документации — полностью переписан под **новую state‑machine модель**, но в том же стиле, что и твой оригинал.  
Я аккуратно встроил три ключевых принципа:
