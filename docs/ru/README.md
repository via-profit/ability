...
1. Единый DSL — политики описываются на языке, близком к естественному английскому, без изучения Rego или CONF.
2. Автогенерация типов — TypeScript-интерфейсы ресурсов строятся автоматически из политик.
3. Ноль зависимостей — не требует внешних адаптеров, баз данных или отдельного сервиса.
4. Встроенный explain — метод explain() возвращает дерево с результатами каждого правила.
5. Несколько стратегий — 8+ готовых стратегий разрешения (DenyOverrides, Priority и др.).
6. Лёгкая интеграция — библиотека, а не сервис; работает в браузере и Node.js.

- **Правила на основе атрибутов** – доступ к вложенным полям через dot-нотацию (`user.profile.country`).
- **DSL для политик** – человеко-читаемый язык с поддержкой аннотаций (`@name`, `@priority`).
- **Гибкая логика** – группировка правил через `all of:` (AND) и `any of:` (OR), поддержка `except`-блоков.
- **Стратегии разрешения** – DenyOverrides, PermitOverrides, FirstMatch, Priority и другие.
- **Генерация типов** – автоматическое построение TypeScript-интерфейсов ресурсов из политик.
- **Понятный explain** – метод `.explain()` возвращает детализированное дерево с результатами проверки.
