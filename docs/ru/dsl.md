...
```dsl
@name "Редактировать заказ может только авторизоавнный пользователь"
permit permission.order.update if all:
  user.token is not null
...
deny permission.order.update if all:
  all of:
    ...
    user.role is equeals 'administrator'
```
...
- Если `user.role equals 'administrator'` → условие `except` истинно → **deny отменяется** → итог **permit**.
...
|---------------------------|--------------------------|--------------------------------|---------------------|-------------------------|
| **is equals**             | `=`, `==`, `is` `equals` | `age is equals 18`             | Строгое равенство   | number, string, boolean |
...
| Оператор DSL       | Синонимы | Пример                           | Описание                             | Типы |
|--------------------|----------|----------------------------------|--------------------------------------|------|
| **is defined**     | -        | `user.middleName is defined`     | Значение определено (не `undefined`) | any  |
